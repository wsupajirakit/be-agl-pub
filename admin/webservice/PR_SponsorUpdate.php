    ...
    if($update == 'info'){
      ...
      if(strlen($prTitle) > 0){
        ...
        if($res!==false){
          ...
          $response = array('status' => 1, 'msg' => $txt_var['request_success'], 'result' => array('pr_id' => $pr_id, 'pr_title' => $prTitle, 'pr_url' => $prUrl, 'pr_image' => $prConfig['sponsor_url'] . $prRow['pr_image']),);
        }else{
      ...
      $prImage = $_FILES['sponsor_image'];

      $uploadDir = __DIR__."/../../rsc/sponsor/";
      $fileExt = getFileExtension($prImage["name"]);
      $newName = randomFileName();
      $targetFile = $uploadDir . $newName . "." . $fileExt;
      $validateFile = validateFileToUpload($prImage, 'image', $uploadDir, $prConfig["sponsor_ext"], $prConfig["sponsor_size_min"], $prConfig["sponsor_size_max"], _FILE_UPLOAD_MAX_SIZE);
      if(1!=$validateFile){
        $response = array('status' => 2, 'msg' => $txt_var['request_error']);
      }else{
        // upload new image
        if(move_uploaded_file($prImage["tmp_name"], $targetFile)===true){
          ...
          if($res!==false){
            ...
            $response = array('status' => 1, 'msg' => $txt_var['request_success'], 'result' => array('pr_id' => $pr_id, 'pr_title' => $prRow['pr_title'], 'pr_url' => $prRow['pr_url'], 'pr_image' => $prConfig['sponsor_url'] . $prRow['pr_image']));
            // delete old image
