    ...
    if(checkdate($tc_month, $tc_day, $tc_year) && in_array($tc_num, $allowTcNum)){
      if(count($songs) > 0 && count($songs) == $tc_num){
        ...
        if($chk == count($songs)){
          ...
          $chkDate = $db->dbCount("SELECT COUNT(*) FROM top_chart WHERE top_chart_date = :dt AND top_chart_id != :id", array(':dt' => $dateFormatted, ':id' => $tc_id));
          if($chkDate == 0){
            $db->pdo->beginTransaction();
            try{

              $sql = "DELETE FROM top_chart_song WHERE top_chart_id = :id";
              $param = array(':id' => $tc_id);
              $stmt = $db->pdo->prepare($sql);
              $stmt->execute($param);

              $sql = "UPDATE top_chart SET top_chart_date = :dt WHERE top_chart_id = :id";
              $param = array(':dt' => $dateFormatted, ':id' => $tc_id);
              $stmt = $db->pdo->prepare($sql);
              $stmt->execute($param);
              ...
              for($i=0; $i < count($songs); $i++){ 
                ...
                $param = array(':tid' => $tc_id, ':sid' => $_sid, ':od' => $i+1, ':lwo' => $_lwo, ':ocwn' => $_ocwn);
                $stmt = $db->pdo->prepare($sql);
                $stmt->execute($param);
              }
              $db->pdo->commit();
