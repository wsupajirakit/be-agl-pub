      ...
      if(count($artists) > 0){
        ...
        if($chk == count($artists)){
          ...
          try{
            ...
            $param = array(':id' => $song_id);
            $stmt = $db->pdo->prepare($sql);
            $stmt->execute($param);

            $sql = "UPDATE song SET song_name = :nm, artist_name_map = :mp WHERE song_id = :id";
            $param = array(':nm' => $song_name, ':mp' => $artistName, ':id' => $song_id);
            $stmt = $db->pdo->prepare($sql);
            $stmt->execute($param);

            for($i=0; $i < count($artists); $i++){ 
              $sql = "INSERT INTO song_artist (song_id, artist_id, is_lead_artist) VALUES(:sid, :aid, :ld)";
              $param = array(':sid' => $song_id, ':aid' => $artists[$i], ':ld' => $i==0 ? 1 : 0);
              $stmt = $db->pdo->prepare($sql);
              $stmt->execute($param);
            }
            $db->pdo->commit();
