<?
  ...
  if($auth->VerfiyJWToken($userAccessToken)){
    $countPopup = $db->dbCount("SELECT COUNT(*) FROM public_relations WHERE pr_type = 4");
    if($countPopup == 0){
      if(strlen($prTitle) > 0){
        ...
        // $nowBkk->setTimestamp(strtotime("now"));
        ...
        $checkImg = true;
        $uploadDir = __DIR__."/../../rsc/app/";
        $fileExt = getFileExtension($prImage["name"]);
        $newName = randomFileName();
        $targetFile = $uploadDir . $newName . "." . $fileExt;
        $validateFile = validateFileToUpload($prImage, 'image', $uploadDir, $prConfig["app_popup_ext"], $prConfig["app_popup_size_min"], $prConfig["app_popup_size_max"], _FILE_UPLOAD_MAX_SIZE);
        if(1!=$validateFile){
          $checkImg = false;
        }else{
        ...
        if($checkImg===true){
          ...
          $param = array(':tt' => $prTitle, ':im' => $prImageName, ':tp' => 4, ':dt' => $nowFormatted, ':pu' => $prUrl, ':uid' => $userAccessTokenData['uid']);
          $res = $db->dbQuery($sql, $param);
          if($res!==false){
            $rowId = $db->lastInsertID();
            $response = array('status' => 1, 'msg' => $txt_var['request_success'], 'result' => array('pr_id' => $rowId, 'pr_title' => $prTitle, 'pr_url' => $prUrl, 'pr_image' => $prConfig["app_popup_url"] . $prImageName));
          }else{
            unlink($targetFile);
